...
class SimpleChatService:
    ...
    def __init__(self, video_id: str = None, lesson_id: str = None):
    ...
    def _search_summaries(self, query: str) -> str:
        ...
        try:
            # Kiểm tra xem có video_id hoặc lesson_id không
            if not self.video_id and not self.lesson_id:
                return "❌ Vui lòng cung cấp video_id hoặc lesson_id để tìm kiếm tóm tắt bài học."
            
            # Tìm kiếm tóm tắt với filter
            results = self.storage.search_summaries(query, top_k=3)
            
            # Lọc kết quả theo video_id hoặc lesson_id
            filtered_results = []
            for result in results:
                metadata = result['metadata']
                result_video_id = metadata.get('video_id', '')
                result_lesson_id = metadata.get('lesson_id', '')
                
                # Kiểm tra match với video_id và lesson_id (cả 2 phải khớp)
                video_match = not self.video_id or result_video_id == self.video_id
                lesson_match = not self.lesson_id or result_lesson_id == self.lesson_id
                
                if video_match and lesson_match:
                    filtered_results.append(result)
            
            if not filtered_results:
                return f"Không tìm thấy tóm tắt bài học nào phù hợp với câu hỏi của bạn trong {'video ' + self.video_id if self.video_id else 'lesson ' + self.lesson_id}."
    ...
    def _search_transcripts(self, query: str, video_id: str = None) -> str:
        ...
        try:
            # Kiểm tra xem có video_id hoặc lesson_id không
            if not self.video_id and not self.lesson_id:
                return "❌ Vui lòng cung cấp video_id hoặc lesson_id để tìm kiếm transcript."
            ...
            if not filtered_results:
                return f"Không tìm thấy nội dung transcript nào phù hợp với câu hỏi của bạn trong {'video ' + self.video_id if self.video_id else 'lesson ' + self.lesson_id}."
            ...
            for i, result in enumerate(filtered_results, 1):
                ...
                transcript_text += f"- Lesson ID: {metadata.get('lesson_id', 'N/A')}\n"
    ...
    def _get_all_videos(self) -> str:
        ...
        try:
            ...
            # Lọc video theo video_id hoặc lesson_id nếu có
            filtered_videos = []
            for video in videos:
                ...
                # Nếu có lesson_id, chỉ lấy video của lesson đó
                elif self.lesson_id:
            ...
            if not filtered_videos:
                filter_text = f"video {self.video_id}" if self.video_id else f"lesson {self.lesson_id}"
            ...
            for i, video in enumerate(filtered_videos, 1):
                ...
                response += f"   **Lesson ID:** {video.get('lesson_id', 'N/A')}\n"
    ...
    def _get_summary_by_video_id(self, video_id: str = None) -> str:
        ...
        try:
            ...
            response += f"**Lesson ID:** {metadata.get('lesson_id', 'N/A')}\n"
    ...
    def _get_subtitle_by_timestamp_id(self, timestamp_id: str, video_id: str = None) -> str:
        ...
        try:
            ...
            prompt = TIMESTAMP_PROMPT.format(
                ...
                lesson_id=results[0]['metadata'].get('lesson_id', 'N/A'),
    ...
    def _search_by_lesson_id(self, lesson_id: str = None) -> str:
        """Tìm kiếm theo lesson ID trong phạm vi video/lesson hiện tại"""
        try:
            target_lesson_id = lesson_id or self.lesson_id
            if not target_lesson_id:
                return "❌ Vui lòng cung cấp lesson_id hoặc khởi tạo chat service với lesson_id"
            ...
            for s in summaries:
                metadata = s['metadata']
                if metadata.get('lesson_id') == target_lesson_id:
            ...
            context = f"**Tìm kiếm theo Lesson ID: {target_lesson_id}**\n\n"
    ...
    def _analyze_query(self, query: str) -> str:
        ...
        # Kiểm tra video_id và lesson_id
        if 'video id' in query_lower or 'video_id' in query_lower:
            return "get_summary_by_video"
        elif 'lesson id' in query_lower or 'lesson_id' in query_lower:
            return "search_by_lesson_id"
    ...
    def chat(self, message: str) -> str:
        ...
        try:
            ...
            if action == "get_all_videos":
            ...
            elif action == "search_by_lesson_id":
                # Extract lesson_id from message if possible
                import re
                lesson_id_pattern = r'(?:lesson[_\s]*id|lesson_id)[\s:]*([a-zA-Z0-9_-]+)'
                lesson_id_match = re.search(lesson_id_pattern, message.lower())
                if lesson_id_match:
                    lesson_id = lesson_id_match.group(1)
                else:
                    lesson_id = self.lesson_id  # Use default from initialization
                response = self._search_by_lesson_id(lesson_id)
                ...
                if timestamp_match:
                    ...
                    if minute_match:
                        timestamp_id = minute_match.group(1)
                    else:
                ...
                if timestamp_id:
                    ...
                    response = "Vui lòng cung cấp timestamp ID cụ thể. Ví dụ: 'timestamp_id: 1', 'phút 2', hoặc chỉ gõ '3'"
